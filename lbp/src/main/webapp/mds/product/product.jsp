...
<head>
	...
	<script type="text/javascript">
		function nextPage_product(start, limit){			
			this.start=start || this.start;
			if(start == 0) this.start = 0;
			this.limit=limit || this.limit;			
			var paramObj={start:this.start, limit:this.limit};
			var form = document.forms["searchForm"];
			url = path+"/mds/product/listProduct.htm";
			doAjaxPost(form, url, paramObj);
		}
		function doSearch(button){		
			action = path+"/mds/product/listProduct.htm";
			return doAjax(button, action);
		}
		function update(id){
			tb_show("变更（标为<span class='high_star'>*</span>必填项）", path+"/mds/product/getProduct.action?flag=update&table=stand&id="+id+"&height=610&width=450");
			$( "#TB_window" ).draggable();
		}
		function detail(id){
			tb_show("明细（标为<span class='high_star'>*</span>必填项）", path+"/mds/product/getProduct.action?flag=detail&table=stand&id="+id+"&height=600&width=450");
			$( "#TB_window" ).draggable();
		}
		function remove(id){
			Asc.windows.confirm('提示','确认删除该产品及其临时表？',function(c){
				if(c=='yes'){
					$.ajax({
					   type:"POST",
					   url:path+"/mds/product/removeProduct.htm",
					   data:"id=" + id,
...
<div class="wrap">
	...
	<div class="admin_table mb10" id="query_area" style="display: block;">
		<s:form name="searchForm" action="listProduct.action" method="post" id="searchForm">
