...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$('#orgName_ac').simpleAutoComplete(path+"/mds/factoryorg/autoCompleteFactoryOrg.action");
			$('#ownerAscName_ac').simpleAutoComplete(path+"/mds/factoryorg/autoCompleteOrg.action");
		});
		function nextPage_factoryOrg(start, limit){			
			this.start=start || this.start;
			if(start == 0) this.start = 0;
			this.limit=limit || this.limit;			
			var paramObj={start:this.start, limit:this.limit};
			var form = document.forms["searchForm"];
			url = path+"/mds/factoryorg/listFactoryOrg.htm";
			doAjaxPost(form, url, paramObj);
		}
		function doSearch(button){		
			action = path+"/mds/factoryorg/listFactoryOrg.htm";
			return doAjax(button, action);
		}
		function update(id){
			tb_show("变更（标为<span class='high_star'>*</span>必填项）", path+"/mds/factoryorg/getFactoryOrg.action?flag=update&id="+id+"&height=460&width=450");
			$( "#TB_window" ).draggable();
		}
		function detail(id,state){
			tb_show("明细（标为<span class='high_star'>*</span>必填项）", path+"/mds/factoryorg/getFactoryOrg.action?flag=detail&id="+id+"&height=540&width=400");
			$( "#TB_window" ).draggable();
		}
		function qualityCheck(id,quality){
			tb_show("质检标签", path+"/mds/factoryorg/getFactoryOrg.action?flag=qualityCheck&id="+id+"&qualityCheck="+quality+"&height=100&width=400");
			$( "#TB_window" ).draggable();
		}
		...
		function remove(id){
			Asc.windows.confirm('提示','确认删该企业及其临时表？',function(c){
				if(c=='yes'){
					$.ajax({
					   type:"POST",
					   url:path+"/mds/factoryorg/removeFactoryOrg.htm",
					   data:"id=" + id,
		...
		<table width="100%;" border="0" cellspacing="0" cellpadding="0" id="companytable">
			...
			<tr class="tr2">
				<td scope="col"><input type="checkbox" name="all_cbx" title="全选 " onclick="checkAll(this, 'code_cbx')" /></td>
		  		...
		  		<td scope="col">省</td>
		  		<td scope="col">市</td>
<!-- 		  		<td scope="col">所属企业ASC_CODE</td> -->
<!-- 		  		<td scope="col">对应企业ASC_CODE</td> -->
...
<%-- 					<td class="td2"><s:property value="partnerAscCode"/></td> --%>
					<td class="td2">
