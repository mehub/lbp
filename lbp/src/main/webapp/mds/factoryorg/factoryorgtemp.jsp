...
<head>
	...
	<script type="text/javascript">
		function nextPage_factoryOrgTemp(start, limit){			
			this.start=start || this.start;
			if(start == 0) this.start = 0;
			this.limit=limit || this.limit;			
			var paramObj={start:this.start, limit:this.limit};
			var form = document.forms["searchForm"];
			url = path+"/mds/factoryorg/listTempFactoryOrg.htm";
			doAjaxPost(form, url, paramObj);
		}
		function doSearch(button){		
			action = path+"/mds/factoryorg/listTempFactoryOrg.htm";
		...
		function upload(){
			tb_show("文件上传 &nbsp;【<a href=templateDownloadUploadFile.action?datatype=4>模板下载</a>】", 
					"#TB_inline?&inlineId=uploadContainer&height=150&width=220");
			$( "#TB_window" ).draggable();
		}
		function update(id){
			tb_show("变更（标为<span class='high_star'>*</span>必填项）", path+"/mds/factoryorg/getTempFactoryOrg.action?flag=update&id="+id+"&height=470&width=450");
			$( "#TB_window" ).draggable();
		}
		function detail(id){
			tb_show("明细（标为<span class='high_star'>*</span>必填项）", path+"/mds/factoryorg/getTempFactoryOrg.action?flag=detail&id="+id+"&height=540&width=400");
			$( "#TB_window" ).draggable();
		}
...
<div class="wrap">
	...
	<div class="admin_table mb10" id="query_area" style="display: block;">
		<s:form name="searchForm" action="listTempFactoryOrg.action" method="post" id="searchForm">
				...
				<tr class="tr1 vt">		 
					...
					<td class="td2">
						...
						<s:if test="audiState.code == 1">
							...
							<span class="c5">
								<s:property value="audiState.value"/>
							</span>
						</s:else>
					</td>
				</tr>
...
<script type="text/javascript">
	...
	$('#submitOperate').bind('click',function(){
		if(confirm("准备提交数据，是否继续？")){
			Mask.show();
			jQuery.ajax({
	  			url:path+'/mds/factoryorg/audiFactoryOrg.action',
	  			type:'post',
	  			dataType:'json',
	  			data: {"id" : $("#id").val(), 
	  				   "relid" : $("#relid").val(), 
