...
<head>
	...
	<script type="text/javascript">
		function nextPage_franchiseOrgTemp(start, limit){			
			this.start=start || this.start;
			if(start == 0) this.start = 0;
			this.limit=limit || this.limit;			
			var paramObj={start:this.start, limit:this.limit};
			var form = document.forms["searchForm"];
			url = path+"/mds/franchiseorg/listTempFranchiseOrg.htm";
			doAjaxPost(form, url, paramObj);
		}
		function doSearch(button){		
			action = path+"/mds/franchiseorg/listTempFranchiseOrg.htm";
			return doAjax(button, action);
		}
		function add(){
			tb_show("新增（标为<span class='high_star'>*</span>必填项）", "update.jsp?height=500&width=450");
			$( "#TB_window" ).draggable();
		}
		function upload(){
			tb_show("文件上传 &nbsp;【<a href=templateDownloadUploadFile.action?datatype=5>模板下载</a>】", 
					"#TB_inline?&inlineId=uploadContainer&height=150&width=220");
			$( "#TB_window" ).draggable();
		}
		function update(id){
			tb_show("变更（标为<span class='high_star'>*</span>必填项）", path+"/mds/franchiseorg/getTempFranchiseOrg.action?flag=update&id="+id+"&height=470&width=450");
			$( "#TB_window" ).draggable();
		}
		function detail(id){
			tb_show("明细（标为<span class='high_star'>*</span>必填项）", path+"/mds/franchiseorg/getTempFranchiseOrg.action?flag=detail&id="+id+"&height=540&width=400");
			$( "#TB_window" ).draggable();
		}
		function audit(id, relid, name){
			getById("audi").reset();
			$("#id").attr("value", id);
			$("#relid").attr("value", relid);
			$("#xname").attr("value", name);
...
<div class="wrap">
	...
	<div class="admin_table mb10" id="query_area" style="display: block;">
		<s:form name="searchForm" action="listTempFranchiseOrg.action" method="post" id="searchForm">
			<table width="100%" cellspacing="0" cellpadding="0">
				<tbody>
					<tr class="tr1 vt">
						<td class="td2">
							所属企业：<s:textfield name="ownerAscName" cssClass="input"></s:textfield>
							名称：<s:textfield name="orgName" cssClass="input"></s:textfield>
	...
	<h2 class="h1 cc" id="h_btns"><span class="mr10 fl">企业列表</span> 
		<span class="fr">
    		<sec:authorize ifAnyGranted="ADMIN, FACTORY_ORG_ADD">
	    		<span class="linka fl mr5">
	    			<a class="btn_add fl" href="javascript:void(0)" onclick="add();"><i>新增</i></a>
	    		</span>
    		</sec:authorize>
    		<sec:authorize ifAnyGranted="ADMIN, FACTORY_ORG_UPLOAD">
    		<span class="linka fl mr5">  
				...
				<tr class="tr1 vt">		 
					...
					<td class="td2">
						...
						<s:if test="audiState.code == 1">
							<a onclick="audit('<s:property value="id"/>', '<s:property value="relid"/>', '<s:property value="orgName"/>')" href="javascript:void(0)">[审核]</a>
						</s:if><s:else><span class="c5"><s:property value="audiState.value"/></span></s:else>
					</td>
				</tr>
			</s:iterator>
			</tbody>
		...
		<table width="100%" cellspacing="0" cellpadding="0">
			<tr class="tr1 vt">
				...
				<td class="td2">
					...
					<input type="text" id="xname" class="input_wa mr10 fl" value="" disabled="disabled" />
				</td>
			</tr>
			<tr class="tr1 vt">
...
<script type="text/javascript">
	...
	$('#submitOperate').bind('click',function(){
		if(confirm("准备提交数据，是否继续？")){
			Mask.show();
			jQuery.ajax({
	  			url:path+'/mds/franchiseorg/audiFranchiseOrg.action',
	  			type:'post',
	  			dataType:'json',
	  			data: {"id" : $("#id").val(), 
	  				   "relid" : $("#relid").val(), 
	  			...
	  			success:function(json){
	  				...
	  				if(Asc.checkCallback(json)){
						...
						Asc.windows.alert('提示',info,function (){
							$('#searchForm').submit();
						});
					}
	  			}
